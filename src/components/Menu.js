                    ...
                    {_.get(this.props, 'pageContext.site.siteMetadata.menu.actions') && 
                        <ul className="actions stacked">
                            {_.map(_.get(this.props, 'pageContext.site.siteMetadata.menu.actions'), (action, action_idx) => (
                                <li key={action_idx}><Link to={(_.get(action, 'url').startsWith('#') ? _.get(action, 'url') : safePrefix(toUrl(this.props.pageContext.pages, _.get(action, 'url'))))} className={classNames('button', 'fit', {'primary': _.get(action, 'is_primary')}, {'scrolly': _.get(action, 'is_scrolly')})}>{_.get(action, 'label')}</Link></li>
                            ))}
                        </ul>
                    }
                </div>
